function getCity() {
    $.getJSON("/country/get-city", {'country': 'TW', 'province': '台灣'}, function (data) {
        $("#city_select option").remove();
        $("#postCode").val("");
        $("#city_select").append("<option value=''>市</option>");
...
function getArea(country, obj) {
    ...
    $.getJSON("/country/get-area", {'country':country, 'province': province, city:$(obj).val()}, function (data) {
        ...
        $("#postCode").val("");
        $("#area_select").append("<option value=''>区</option>");
        $.each(data, function(e,v){
            $("#area_select").append("<option value='" + v.post_code + "'>" + v.area +"</option>");
        });
    })
}

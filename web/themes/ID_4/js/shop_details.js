...
$(function (){
    ...
    $.ajax({
        ...
        success: function (res){
            ...
            $('.details-title').text(_product.sale_info + " " + _product.title);
            //price
            var salePrice = toThousands(_product.sale_price);
            nowPriceHtml = '<span class="small-price">' + currency + '</span>\
                        <span id="nowPrice">' + salePrice + '</span>\
                        <span class="fr"><i class="this_i"></i>จำนวนที่เหลือ100</span>';
            $('.detail-base-info .info .retail-price').html(nowPriceHtml);
            oldPriceHtml = '<span>ราคาเดิม</span>\
                           <del>' + currency + toThousands(_product.price) + '</del>';
            $('.detail-base-info .info .line2 .market_price_onr').html(oldPriceHtml);
            //info
            $('#info').html(_product.info);
            //图片加载失败，添加默认图片
            $('#swiperWrapper img, #info img').error(function (){
                $(this).attr('src', '/themes/ID_4/images/banner1.jpg');
            });
    ...
    //返回顶部
    var $content = $('.page').eq(0).find('.content');
    var $mGoToTop = $('.slideButton .m-goToTop');
    var scrollTopNum = 0;
    $content.scroll(function (){
        scrollTopNum = $content.scrollTop()
        if (scrollTopNum >= 100){
            $mGoToTop.addClass('show');
        } else {
            $mGoToTop.removeClass('show');
        }
    });
    $mGoToTop.click(function (){
        var scrollTopSer = setInterval(function (){
            scrollTopNum -= 50;
            if (scrollTopNum <= 0){
                scrollTopNum = 0;
                clearInterval(scrollTopSer);
            }
            $content.scrollTop(scrollTopNum);
        }, 1);
    });
    //提交
